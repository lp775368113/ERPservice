...
public class CommonServiceImpl implements CommonService {
	...
	@Override
	public String getQueryDIYData(String name2) {
		System.out.println(name2);
		System.out.println("-----------------");
		JSONObject jsonstr = JSONObject.fromObject(name2);
		Para stu=(Para)JSONObject.toBean(jsonstr, Para.class);
		System.out.println(stu);
		HashMap<String, Object> result=new HashMap<String, Object>();
	...
	private String getDIYSQL(Para stu) {
		String[] xhs=stu.getXh().split(",");
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
	       String startdate=stu.getStarttime();
	       String enddate=stu.getEndtime();
	       StringBuffer sql=new StringBuffer();
	        sql.append(" select g.prod_code,g.prod_name,sum(shuliang) shuliang,sum(xiaoliang) xiaoliang from ( SELECT ");
	        sql.append(" PROD_CODE,PROD_NAME,TO_CHAR ( IN_DATE, 'YYYY-MM' ),SUM( PARTS_ORDERED ) shuliang ,SUM( AMOUNT )  xiaoliang ");
	        sql.append(" FROM ");
	        sql.append(" ( ");
	        sql.append(" SELECT ");
	        ...
	        sql.append(" GROUP BY ");
	        sql.append(" PROD_CODE,PROD_NAME, ");
	        sql.append(" TO_CHAR ( IN_DATE, 'YYYY-MM' ) ) g  group by g.prod_code,g.prod_name ");
		System.out.println("本月销量sql："+sql);
		return sql.toString();
	}
}
